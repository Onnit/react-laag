...
function useOnScroll(elements, onScroll, environment, trackScroll) {
  ...
  var memoElements = useMemo(function () {
...
function useStyleState(anchor) {
  ...
  var _React$useState2 = useState(INITIAL_STYLES),
      styles = _React$useState2[0],
      setStyles = _React$useState2[1];

  var lastStyles = useRef(styles);
  return {
    styles: styles,
...
function getLayerRectByAnchor(_ref4) {
  ...
  var result = _extends({}, layerRect, {}, primaryRect, {}, secondaryRect); // correct scrollOffsets
...
function getVisibleLayerSurface(layer, parent) {
  ...
  var _ALL_OFFSET_SIDES$fil = ALL_OFFSET_SIDES.filter(function (side) {
    ...
    return _extends({}, rect, (_extends2 = {}, _extends2[affectedProperty] = rect[affectedProperty] + offsets[side], _extends2));
  }, layer),
...
function getPositioning(_ref) {
  ...
  var _getAbsoluteStyle = getAbsoluteStyle(_extends({
    rects: rects
  }, options)),
...
function useElementRef(initialState, elementToState) {
  var _React$useState = useState(initialState || null),
...
function useElementState(container, fixed, environment) {
  return useElementRef({
    ...
    scrollParents: []
  }, useCallback(function (triggerElement) {
...
function useIsOpen(internal, external) {
  var shouldOpenAfterMount = useRef(external);
  var isOpen = shouldOpenAfterMount.current ? false : isSet(external) ? external : internal;
  var rerenderAfterMount = useState(false)[1];
  useEffect(function () {
...
function ToggleLayer(_ref) {
  ...
  var handlePositioning = useCallback(function () {
    if (!triggerElement) {
  ...
  var outsideClickRefs = useRef(new Set([layerRef, normalTriggerRef])); // handle clicks that are not originated from the trigger / layer
  // element

  useOutsideClick(outsideClickRefs, useCallback(function () {
    if (!isOpen) {
      return;
    }
...
function useToggleLayer(renderLayer, _ref) {
  ...
  var _ref2 = _ref,
      ...
      props = _objectWithoutPropertiesLoose(_ref2, ["onStyle", "closeOnOutsideClick", "closeOnDisappear", "fixed", "container", "placement", "environment"]);
  ...
  var layerRef = useRef(null);
  var triggerRectRef = useRef(null);
  ...
  var element = relativeParentElement && createPortal(renderLayer({
    layerProps: {
      ref: function ref(element) {
        if (element) {
          // observe the layer for resizing
...
function useBreakpoint(maxPixels) {
  var _React$useState = useState(typeof window !== "undefined" ? window.matchMedia("(max-width: " + maxPixels + "px)").matches : false),
...
function useTooltip(renderLayer, _ref) {
  ...
  var _ref2 = _ref,
      ...
      rest = _objectWithoutPropertiesLoose(_ref2, ["delayEnter", "delayLeave", "hideOnScroll"]);
